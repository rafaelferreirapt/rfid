...
class ObjectsTestCase(TestCase):
    ...
    def test_products(self):
        ...
        url = "/api/v1/category/search/" + self.sub_hall12.name + "/" + str(self.category3.id) + "/"
        response = client.get(path=url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 5)
        self.assertEqual(response.data[0]["name"], self.sub_hall12.name)
        self.assertEqual(response.data[1]["name"], self.sub_hall11.name)
        self.assertEqual(response.data[2]["name"], self.sub_hall10.name)
        self.assertEqual(response.data[3]["name"], self.sub_hall18.name)
        self.assertEqual(response.data[4]["name"], self.sub_hall6.name)
